...
async function loadLookupMenu() {
    const response = await fetch('/api/lookup-menu');
    const data = await response.json();
    const selectElement = document.getElementById('menu-select'); // HTML'de select elemanı ekleyin

    if (data.success) {
        data.data.forEach(item => {
            const option = document.createElement('option');
            option.value = item.id;
            option.textContent = item.yemek_adi;
            selectElement.appendChild(option);
        });
    }
}

document.addEventListener('DOMContentLoaded', loadLookupMenu);
