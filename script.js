...
async function loadMenuReservations(menuId) {
    const response = await fetch(`/api/menu-reservations/${menuId}`);
    const data = await response.json();
    const tableBody = document.querySelector('#reservation-table tbody');
    ...
    if (data.success) {
        ...
        data.data.forEach(reservation => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${reservation.ad_soyad}</td>
                <td>${reservation.tarih}</td>
                <td>${reservation.ogun}</td>
                <td>${reservation.bakiye}</td>
            `;
            tableBody.appendChild(row);
        });
    }
