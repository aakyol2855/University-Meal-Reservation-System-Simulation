...
app.get('/api/lookup-menu', async (req, res) => {
    try {
        const menuList = await db.query('SELECT id, yemek_adi FROM yemek_tablosu');
        res.json({ success: true, data: menuList });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Menu bilgisi alınamadı.' });
    }
});
app.get('/api/menu-reservations/:menuId', async (req, res) => {
    const { menuId } = req.params;
    try {
        const reservations = await db.query(
            'SELECT * FROM rezervasyon_tablosu WHERE menu_id = ?',
            [menuId]
        );
        res.json({ success: true, data: reservations });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Rezervasyon bilgisi alınamadı.' });
    }
});
app.post('/api/add-reservation', async (req, res) => {
    const { menuId, userId, tarih, ogun, bakiye } = req.body;
    try {
        await db.query(
            'INSERT INTO rezervasyon_tablosu (menu_id, user_id, tarih, ogun, bakiye) VALUES (?, ?, ?, ?, ?)',
            [menuId, userId, tarih, ogun, bakiye]
        );
        res.json({ success: true, message: 'Rezervasyon başarıyla eklendi.' });
    } catch (error) {
...
app.get('/api/user-count', async (req, res) => {
    try {
        const [result] = await db.query('SELECT COUNT(*) AS totalUsers FROM kullanici_tablosu');
        res.json({ success: true, totalUsers: result.totalUsers });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Kullanıcı sayısı alınamadı.' });
    }
});
